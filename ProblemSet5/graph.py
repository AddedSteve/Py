    ...
    def __str__(self):
        ...
        for k in self.edges:
            for d, edge in self.edges[k]:
                output = '{0}{1}->{2} ({3}, {4})\n'.format(output, k, d, float(edge.getTotalDistance()), float(edge.getOutdoorDistance()))
        return output[:-1]
...
class WeightedEdge(Edge):
    def __init__(self, src, dest, totalDistance, outdoorDistance):
        ...
        self.totalDistance = int(totalDistance)
        self.outdoorDistance = int(outdoorDistance)
    ...
    def __repr__(self):
        return '({0}, {1})'.format(self.totalDistance, self.outdoorDistance)
