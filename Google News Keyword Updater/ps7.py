...
class NewsStory(object):
    def __init__(self, guid0, title0, subject0, summary0, link0):
...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, text):     
        if self.phrase in text.getTitle():
            return True
        elif self.phrase in text.getSubject():
            return True
        elif self.phrase in text.getSummary():
            return True
        else:
            return False
...
def filterStories(stories, triggerlist):
    ...
    for story in stories:
        if story.getGuid() not in checker:
            for trigger in triggerlist:
                if trigger.evaluate(story):
                    filteredStories[i] = story
                    i+=1
                    checker += story.getGuid()
                    break
                    
                    
    filteredStories = filteredStories.values()
